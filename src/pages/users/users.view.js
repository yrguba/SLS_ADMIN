...
function UsersView(props) {
  ...
  const [userDeviceUnlinkModalVisible, setUserDeviceUnlinkModalVisible] =
    useState(false);
  const [currentUser, setCurrentUser] = useState(null);
  const [currentDeviceId, setCurrentDeviceId] = useState(null);
  const [permissionsModalVisible, setPermissionsModalVisible] = useState(false);
  const [checkedPermissionsOptions, setCheckedPermissionsOptions] =
    useState(permissionsOptions);
  ...
  const usersColumns = [
    {
      ...
      render: (value) => {
        return (
          ...
          >
            <Avatar size={40} name={value.name} round={true} />
            <span
              style={{
                marginLeft: "10px",
              }}
    ...
    <Wrapper {...props} tabActive="users">
      ...
      >
        {currentUser && (
          <div className="user-info">
            <div>
              <Avatar name={currentUser.name} round={true} size={40} />
              <span
                style={{
                  marginLeft: "10px",
                }}
      ...
      <Modal
        ...
        footer={[
          <Button
            onClick={() => {
              props.changePermissions(
                currentUser.id,
                checkedPermissionsOptions
              );
      ...
      >
        ...
        >
          {props?.permissions?.map((p) => (
            <Checkbox style={{ width: "50%", margin: 0 }} value={p.scope}>
              {p.description}
            </Checkbox>
          ))}
        </Checkbox.Group>
